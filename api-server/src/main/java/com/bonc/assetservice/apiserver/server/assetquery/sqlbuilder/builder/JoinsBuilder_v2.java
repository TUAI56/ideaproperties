...
@Component
public class JoinsBuilder_v2 extends Builder {

    public void build(List<DataModelBase.QueryTable> queryTables, List<Condition_v2> conditions, String proCode, String provId ,String areaCode, String areaId ,  PlainSelect plainSelect, List<QueryFrom> queryFroms, QueryFrom mainTable) {


        //如果是inner join模式
        if (Enums.SqlBuilderModel.INNER_MODE.getValue().equalsIgnoreCase(MODEL)) {
            plainSelect.setJoins(buildJoins4Condition(buildJoins(queryFroms, mainTable), mainTable,queryTables,conditions,proCode,provId,areaCode,areaId));
    ...
    private List<Join> buildJoins(List<QueryFrom> queryFroms, QueryFrom mainTable) {
        ...
        for (QueryFrom from : queryFroms) {
            if (!from.getAlias().equals(mainTable.getAlias())) {
                ...
                joins.add(new Join()
                        .withInner(QueryFrom.JOIN_TYPE_INNER.equals(from.getJoinType()))
     ...
     */
    private List<Join> buildJoins4Condition(List<Join> joins, QueryFrom mainTable, List<DataModelBase.QueryTable> queryTables, List<Condition_v2> conditions, String provCode, String provId, String areaCode, String areaId) {
        //设置ConditionList
//        List<Condition_v2> conditions = dataModel.getConditionV1s();
        if (null != conditions && !conditions.isEmpty()) {


            SelectBody consituionSelectBody = buildConditionsExpression(conditions, queryTables, provCode, provId,areaCode, areaId);
            //获取第一个匹配的参数
            if (null != consituionSelectBody) {
                String queryFiledAndConditionRelation = getQueryFiledAndConditionRelation(queryTables,conditions);
                if (joins.isEmpty()) {
     ...
     * </p>
     *
     * @param conditions condition集合
     * @return SelectBody
     * @author zhaozesheng
     * @since 2022-11-09 15:03:59
     */
    public SelectBody buildConditionsExpression(List<Condition_v2> conditions, List<DataModel_v2.QueryTable> queryTables, String provCode, String provId,String areaCode, String areaId) {

        if (null == conditions || conditions.isEmpty()) return null;
        ...
        for (int i = 0; i < conditions.size(); i++) {
            ...
            //获取子查询
            List<Condition_v2> subConditions = conditions.get(i).getSubConditions();

            //第一个i 不判断操作符，从第二个开始
            if (i > 0) operation.add(createSetOperation(conditions.get(i)));
            //不为空
            if (null == subConditions || subConditions.isEmpty()) {
                //设置planSelect
               selects.add(buidPlanSelect(conditions.get(i),conditions, queryTables, provCode, provId,areaCode,areaId));
            } else {
                //递归子查询
                SelectBody selectBody_sub = buildConditionsExpression(subConditions, queryTables, provCode, provId,areaCode,areaId);
    ...
    private String getQueryFiledAndConditionRelation(List<DataModel_v2.QueryTable> queryTables, List<Condition_v2> conditions) {

        for (DataModel_v2.QueryTable queryTable : queryTables) {
            for (Condition_v2 condition : conditions) {
                if (null != condition.getSubConditions() && !condition.getSubConditions().isEmpty()) {
                    for (Condition_v2 subcCondition : condition.getSubConditions()) {
                        if (subcCondition.getQueryField().getTableCode().equals(queryTable.getTableCode())) {
                            return queryTable.getRelationFieldCode();
                        }
                    }
                }
                if (condition.getQueryField().getTableCode().equals(queryTable.getTableCode())) {
                    return queryTable.getRelationFieldCode();
                }
            }
        }
     ...
     */
    private  PlainSelect buidPlanSelect(Condition_v2 condition, List<Condition_v2> conditions, List<DataModel_v2.QueryTable> queryTables, String provCode, String provId, String areaCode, String areaId) {
        ...
        if ("asset".equals(condition.getConditionType())) {
            ...
            //封装默认查询参数 User_ID
            Expression expression = new Column(getReleAndAcct(condition, queryTables).get(USER_ID));
            ...
            tableName.add(condition.getQueryField().getTableCode());
            ...
            leftExpression.setLeftExpression(new Column(getReleAndAcct(condition, queryTables).get(ACCT)));
            leftExpression.setRightExpression(new StringValue(condition.getQueryField().getAsset().getAcct()));
            ...
            //封装另外的查询条件
            Enums.CompareOperator compareOperator = condition.getCompareOperator();
            Column left = new Column();
            left.setColumnName(condition.getQueryField().getAsset().getAssetCode());
            Object value = condition.getOperand();
        ...
        if ("dataset".equals(condition.getConditionType())) {
            ...
            for (DataModelBase.QueryTable queryTable : queryTables) {
                if (queryTable.getTableCode().equals(condition.getQueryField().getTableCode())) {
                    joinType = queryTable.getRelationFieldCode();
                    break;
                }
            }

            Expression expression = new Column(joinType);
            selectExpressionItem.setExpression(expression);
            //设置自定义数据集的别名为主表的别名
            selectExpressionItem.setAlias(new Alias(getJoinCode(queryTables,conditions)));
            selectItems.add(selectExpressionItem);
            plainSelect.setSelectItems(selectItems);
            ...
            List<QueryField_v2.Column> columns = condition.getQueryField().getDataSet().getColumns();
            if (null == columns || columns.isEmpty()) return plainSelect;


            AndExpression andExpression = new AndExpression();
            Expression dataSetConditionWhereExecpermission = getDataSetConditionWhereExecpermission(columns, andExpression);
            plainSelect.setWhere(dataSetConditionWhereExecpermission);
            return plainSelect;
        }
        return plainSelect;
    }
        ...
        for (int i = 0; i < columns.size(); i++) {
            ...
            if (columns.size() == 1) {
                ...
                expression = Enums.LogicalOperator.OR.equals(columns.get(i).getLogicalOperator())
                        ? new OrExpression(comparisonOperator, getDataSetConditionWhereExecpermission(columnsChildren, expression))
                        : new AndExpression(comparisonOperator, getDataSetConditionWhereExecpermission(columnsChildren, expression));
                return expression;
            }
...
//        List<Condition_v2> conditions = dataModel.getConditionV1s();
     ...
     */
    private SetOperation createSetOperation(Condition_v2 condition) {
        Enums.LogicalOperator logicalOp = condition.getLogicalOp();
        switch (logicalOp) {
            case OR:
                return new UnionOp();
            case AND:
     ...
     */
    private Map<String, String> getReleAndAcct(Condition_v2 condition, List<DataModel_v2.QueryTable> queryTables) {
        //获取condition中查询的表
        String tableCode = condition.getQueryField().getTableCode();
        //封装返回结果
