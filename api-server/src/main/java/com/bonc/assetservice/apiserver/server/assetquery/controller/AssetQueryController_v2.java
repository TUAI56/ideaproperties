...
import com.bonc.assetservice.apiserver.server.assetquery.parser.ParserManager_v2;
...
import com.bonc.assetservice.apiserver.server.assetquery.validator.ValidatorManager_v2;
...
public class AssetQueryController_v2 extends BaseController {

    @Autowired
    SqlBuilder_v2 sqlBuilder_v2;
    @Autowired
    ValidatorManager_v2.ValidateSync2_ validateSync;

    @Autowired
    ValidatorManager_v2.ValidateAsync2_ validateAsync;


    @Autowired
    ParserManager_v2.SyncParseReq syncParseReq;
    @Autowired
    ParserManager_v2.AsyncParseReq asyncParseReq;

    @Resource(name = "assetQueryServiceImpl_v2")
    ...
    public  CommonResult<SyncAssetQueryRespVO> syncGetData(HttpServletRequest httpReq,
                                                                        @Valid @RequestBody SyncAssetQueryReqVO_v2 req,
        ...
        try {
            ...
            //入参解析成内部结构
            DataModel_v2 dataModel = syncParseReq.parser(req);

            //获取数据
            SyncAssetQueryRespVO ret = assetQueryService.getData(dataModel);
    ...
    public  CommonResult<?> asyncGetData(HttpServletRequest httpReq,
                                                      @Valid @RequestBody AsyncAssetQueryReqVO_v2 req,
                                                      BindingResult bindingResult) {
        ...
        try {
            ...
            //入参解析成内部结构
            DataModel_v2 dataModel = asyncParseReq.parser(req);
            //生成查询sql
            PlainSelect plainSelect = sqlBuilder_v2.buildPlainSelect(dataModel);
    ...
    public  CommonResult queryAsyncAssetStatus(HttpServletRequest httpReq,@RequestParam String reqId, @RequestParam String downFileReqId){

        String tqAppUUId = httpReq.getHeader("appUUid");

        LambdaQueryWrapper<AsyncAssetqueryReq> wrapper = new LambdaQueryWrapper();
        wrapper.eq(AsyncAssetqueryReq::getReqId, reqId);
     ...
     * @return id
     **/
    private Long saveAsyncAssetqueryReq(AsyncAssetQueryReqVO_v2 req, Integer stats) {
